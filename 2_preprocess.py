...
def extract_features(ztf_id, df, label, redshift):
    ...
    for fid, band in [(1, 'g'), (2, 'r')]:
        ...
        total_time_span = df['mjd'].max() - df['mjd'].min()
        observed_duration = max(res.get('g_duration', 0), res.get('r_duration', 0))
        # If the event only happens once, the span and duration should be similar.
        # If it flickers over years, the ratio will be very small.
        res['burst_ratio'] = observed_duration / max(total_time_span, 1.0)                                                                                                          
        ...
        if len(b) > 3:
            ...
            diffs = np.abs(np.diff(b[mag_col].values))
            res[f"{band}_stability"] = np.mean(diffs)
        else:
            res[f"{band}_stability"] = 99 # Penalty for too few points
    
    if not np.isnan(res.get("g_peak")) and not np.isnan(res.get("r_peak")):
