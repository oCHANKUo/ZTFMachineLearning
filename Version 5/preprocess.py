...
def extract_features(ztf_id, df, label, redshift, strict_mode=False):
    """
    Extract features with optional strict quality cuts.

    strict_mode=True: Only keep well-sampled objects with complete light curves.
    This is for anomaly detection - we want to find PHYSICAL anomalies, not data artifacts.

    strict_mode=False: Keep everything (for comparison or when data is precious).
    """
    ...
    # ============================
    # STRICT QUALITY CUTS (for anomaly detection)
    # ============================
    if strict_mode:
        # Reject objects with huge gaps
        if max_gap > 100:
            return None

        # Reject very sparse objects
        if n_total < 20:
            return None

        # Reject imputed-heavy objects
        if frac_imputed > 0.1:
            return None

        # Need both bands reasonably sampled
        if n_g < 8 or n_r < 8:
            return None

        # Need complete rise and fall (peak not at edge)
...
def run_features(use_imputed=False, strict_mode=False):
    ...
    for ztf_id, df in all_lcs.items():
        ...
        if feat is not None:
            ...
            failed += 1
            if strict_mode:
                # Check if it failed due to strict cuts
                feat_loose = extract_features(ztf_id, df, label, redshift, strict_mode=False)
                if feat_loose is not None:
                    rejected_strict += 1
    ...
    out_file = f"ztf_features_{mode_str}.csv"
    ...
    if strict_mode:
        print(f"Rejected by strict cuts: {rejected_strict}")
...
if __name__ == "__main__":
    ...
    print("=== LOOSE MODE (all data) ===")
    run_features(use_imputed=True, strict_mode=False)

    print("\n=== STRICT MODE (clean data only) ===")
    run_features(use_imputed=True, strict_mode=True)